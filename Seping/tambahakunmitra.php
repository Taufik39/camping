...
if(!empty($_POST["daftarmitra"])) {
	...
	$result = $stmt->execute( array(':nama'=>$_POST['nama'],':username'=>$_POST['username'], ':pass'=>$_POST['pass'], ':email'=>$_POST['email'], ':nohp'=>$_POST['nohp'], ':mitra'=>$_POST['mitra'] , ':alamat'=>$_POST['alamat'] , ':profil'=>$_POST['profil'] ) );
	if (!empty($result) ){
